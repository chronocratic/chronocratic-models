...
| MHCCL | Yes | Yes | VECTOR via mean over time, SEQUENCE via transpose |
...
1. **`BasicEncodingMixin`** (`_mixin/encoding.py`) — Fixed-length sequence models (TST, TimeVAE, TimeNet, RecurrentAutoEncoder, MCL, TSTCC, SimCLR, MHCCL, Series2Vec). Subclasses implement `_get_encoder()` and optionally override `_encode_batch()`. The mixin owns DataLoader iteration, eval/inference mode, device placement, and result concatenation.
...
- **MHCCL** — ported as a 1-D ResNet-18 pair sharing `Conv1dResNetEncoder`, for the reasons given under SimCLR. Two departures are structural. First, FINCH clustering is reimplemented in torch on the input's device: the reference uses scikit-learn, SciPy and pynndescent, none of which this library depends on, and a host round-trip per level would dominate a per-batch clustering step; component labelling is asserted equal to `scipy.sparse.csgraph` in `tests/unit/test_mhccl.py`. Second, the reference clusters the whole training set once per epoch and addresses the assignments through a dataset-global sample index that this library's batch contract does not carry, so clustering runs per batch over the raw and augmented embeddings unioned with a momentum feature bank; that bank is capped at one epoch of enqueued rows, because copies of one sample taken from different encoder states cluster together and add a partition encoding duplication rather than structure. Ten further departures, each commented at its line: candidate scores are inner products via `einsum("bd,bmd->bm", ...)`, where the reference reshapes its `(B, M, D)` stack to `(B, D, M)` — reinterpreting the buffer instead of permuting axes, so its einsum contracts mismatched elements; negatives are every instance outside the anchor's cluster, where the reference excludes only the positives it sampled and so leaves a cluster's unsampled members as negatives for an anchor sharing their cluster; candidate slots a batch cannot fill are masked out of the mean rather than raising; the cluster-level term averages over the partitions actually realized, where the reference divides by the configured count and ties the loss scale to the batch's geometry; centroids neither mutate the caller's feature matrix nor mis-count, where the reference zeroes rows in place and subtracts a constant one from every cluster; the level-to-level cluster map comes from the merge step rather than a scan of the label matrix per anchor per candidate; shuffle-BN and the `torch.distributed` dependency are dropped, being the identity at world size 1; normalization defaults to `GroupNorm(1, C)` rather than the reference's `BatchNorm`, so the encoder stays well-defined at `batch_size=1`; views come from the injected producer per batch, where the reference freezes both at `Dataset` construction and never seeds `np.random`; and where the reference's CLI defaults and its published command disagree, the published command wins, setting `use_projection_mlp` and `use_lr_scheduler`.
- **MHCCL, open limit** — the number of partitions the clustering realizes depends on the pool size and on the data's geometry, so it can fall below `hierarchy_levels`. The cluster-level term then averages over the partitions actually used, which is reported as `mhccl/hierarchy_levels_used` and warns once; whether a per-batch pool plus a feature bank is a good proxy for the reference's per-epoch corpus prototypes is empirical and unmeasured.
