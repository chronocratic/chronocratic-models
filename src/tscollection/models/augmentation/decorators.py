...
class Seeded[V]:
    ...
    Uses ``torch.random.fork_rng()`` so that inner randomness is isolated
    from the outer process random state. Repeated calls with the same seed
    produce identical output tensors.
