...
def load_config(config_filepath):
    ...
    """
    with open(config_filepath) as cnfg:  # noqa: PTH123
...
def export_performance_metrics(filepath, metrics_table, header, book=None, sheet_name='metrics'):
    ...
    book.save(filepath)
    logger.info(f"Exported per epoch performance metrics in '{filepath}'")
...
def register_record(
    ...
    if not os.path.exists(filepath):  # Create a records file for the first time
        logger.warning(f"Records file '{filepath}' does not exist! Creating new file ...")
        ...
        try:
            ...
            logger.error(f"Failed saving in: '{filepath}'! Will save here instead: {alt_path}")
            export_record(alt_path, row_values)
            filepath = alt_path

    logger.info(f"Exported performance record to '{filepath}'")
...
def check_model(model, verbose=False, zero_thresh=1e-8, inf_thresh=1e6, stop_on_error=False):
    ...
    for name, param in model.named_parameters():
        ...
        if not param_ok:
            status_ok = False
            print(f"Parameter '{name}' PROBLEM")
        grad_ok = True
        if param.grad is not None:
            grad_ok = check_tensor(
                param.grad, verbose=verbose, zero_thresh=zero_thresh, inf_thresh=inf_thresh
            )
        if not grad_ok:
            status_ok = False
            print(f"Gradient of parameter '{name}' PROBLEM")
        if stop_on_error and not (param_ok and grad_ok):
            if ipdb is None:
                msg = 'ipdb is required when stop_on_error=True.'
...
def check_tensor(X, verbose=True, zero_thresh=1e-8, inf_thresh=1e6):
    ...
    if is_nan.any():
        print(f'{torch.sum(is_nan)}/{X.numel()} nan')
        return False
    ...
    if verbose:
        print(f'Shape: {X.shape}, {X.numel()} elements')
        print("No 'nan' values")
        print(f'Min: {torch.min(X)}')
        print(f'Median: {torch.median(X)}')
        ...
        for b in range(len(binedges) - 1):
            print(f'[{binedges[b]}, {binedges[b + 1]}): {hist[b]}')

        print(f'{num_small}/{X.numel()} abs. values < {zero_thresh}')
        print(f'{num_large}/{X.numel()} abs. values > {inf_thresh}')
