...
def _compute_contrastive_loss_logits(
    combined_instance: torch.Tensor, dimension_size: int, indexing_factor: torch.Tensor
) -> torch.Tensor:
    """
    ...
    indexing_factor : torch.Tensor
        The index used to compute the loss. Kept on device to avoid
        CPU/GPU/MPS transfer overhead and cross-device indexing failures.
...
def temporal_contrastive_loss(instance_1: torch.Tensor, instance_2: torch.Tensor) -> torch.Tensor:
    ...
    indexing_factor = torch.arange(sequence_length, device=instance_1.device)
...
def instance_contrastive_loss(instance_1: torch.Tensor, instance_2: torch.Tensor) -> torch.Tensor:
    ...
    indexing_factor = torch.arange(batch_size, device=instance_1.device)
