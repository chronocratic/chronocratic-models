...
- [ ] **CLN-01**: `enums.py` and `factories.py` removed from augmentation module
- [ ] **CLN-02**: `_sources/rbspaper/` directory removed after all merges complete
- [ ] **CLN-03**: Public module names use clean paths (no underscore prefixes on `abstract/`, `augmentation/`)
- [ ] **CLN-04**: All imports resolve without warnings; `ty check` passes on full source

### Verification

- [ ] **VER-01**: Smoke test: TS2Vec + CropShiftAugmentation trains 5 steps, loss finite
- [ ] **VER-02**: Smoke test: CoST + CosTRandomFunctionAugmentation trains 5 steps, loss finite
- [ ] **VER-03**: Smoke test: AutoTCL + AutoTCLNeuralNetworkAugmentation(RIPTrainingStrategy) trains 5 steps
- [ ] **VER-04**: Extension test: trivial identity `AugmentationMethod` subclass works with any model
- [ ] **VER-05**: Checkpoint round-trip: save → reload → encoder weights match
