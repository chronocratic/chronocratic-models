...
`TrainableAugmentationProducer`. AutoTCL continues to gate its training path with
`isinstance(self._augmentation, TrainableAugmentationProducer)`.
...
**AutoTCL** — needs a single view; may be trainable:
```python
class AutoTCL:
    ...
    def _aug_view(self, x: torch.Tensor) -> torch.Tensor:
        return self._augmentation.produce(x).view
    # trainable path still gated by isinstance(self._augmentation, TrainableAugmentationProducer)
```
