...
**Goal:** Ensure every model exposes a `representation_dim` property that accurately reflects the output of `encode(VECTOR)`, and that users can predict/control the encoding dimension from constructor parameters. Eliminate hidden multipliers (like Series2Vec's `2*D`), missing properties, and property-to-output mismatches.

**Requirements** (from audit findings):

1. **TST**: Fix `representation_dim` mismatch — currently returns `hidden_dims * sequence_length` (80) but `encode(VECTOR)` returns `(B, hidden_dims)` (8). Property must match `encode()` output: return `hidden_dims`.
2. **MCL**: Add `representation_dim` property returning `output_dims` (what `encode()` produces). Document that `forward()` goes through `proj_head` (`projection_dims`) and is not the representation path.
3. **TimeVAE**: Add `representation_dim` returning `latent_dim`. Add `supported_outputs` declaration for SEQUENCE fallback.
4. **TimeNet**: Add `representation_dim` returning `hidden_dims`.
5. **RecurrentAE**: Add `representation_dim` returning `layers[-1]`.
6. **CoST**: Add `representation_dim` returning `output_dims`.
7. **TS2Vec (dilated)**: Add `representation_dim` returning `output_dims`.
8. **AutoTCL**: Add `representation_dim` returning `output_dims`.
9. **TSTCC**: Already correct — no changes needed.
10. **Series2Vec**: Already fixed (Phase 12 prep commit) — `representation_dim` returns `representation_dims`.
11. **Consistency**: Ensure `_encode_batch` docstrings across all models document output shape in terms of constructor params.
12. **Tests**: Update `test_backbone_representation_dim.py` to cover all 10 models and verify `representation_dim` matches actual `encode(VECTOR)` output.
...
*Last updated: 2026-07-03 — Phase 12 added*
