...
### Phase 10: Unified Encoding Output-Shape Contract

**Goal:** Standardize encode() output shape across all 10 models via `EncodingOutputShape` enum with `output=VECTOR`/`SEQUENCE` parameter. Single public knob, same default everywhere. 3-tier classification: Tier A (native-reduce: TSTCC, TimeNet, RecurrentAE, CoST, TS2Vec, AutoTCL), Tier B (native-3D: TST), Tier C (flat-permissive: Series2Vec, MCL, TimeVAE). `supported_outputs` capability declaration, fail-soft warnings on Tier C, producer-side rank asserts.

**Requirements:** encoding-output-shape-contract spec (`.planning/todos/specs/encoding-output-shape-contract.md`)
...
- [ ] 10-01-PLAN.md — Create EncodingOutputShape enum + models public API export (D-01, Wave 1)
- [ ] 10-02-PLAN.md — Split utils.py to utils/ package + _warn_sequence_fallback helper (D-02, Wave 1)
- [x] 10-03-PLAN.md — BasicEncodingMixin output param + rank assert + supported_outputs (D-03, Wave 1)
- [ ] 10-04-PLAN.md — Dilated mixin output param + encoding_window mapping + CoST SEQUENCE concat (D-03, Wave 1)
- [x] 10-05-PLAN.md — Tier A standard models: TSTCC, TimeNet, RecurrentAE (Wave 2)
- [x] 10-06-PLAN.md — Tier A dilated models: CoST, TS2Vec, AutoTCL (Wave 2)
- [x] 10-07-PLAN.md — Tier B model: TST mean-pool VECTOR (Wave 2)
- [x] 10-08-PLAN.md — Tier C models: Series2Vec, MCL, TimeVAE (Wave 2)
- [ ] 10-09-PLAN.md — Tests, existing test updates, CHANGELOG, graphify (Wave 3)

### Phase 11: Model Audits and Fixes

**Goal:** Audit all models for gradient safety, integration issues, and correctness bugs discovered during downstream use (forecasting attacks, batch_size=1 pipelines). Fix identified issues across the board.
**Requirements**: Series2Vec `_distance_normalizer` gradient break at batch_size=1; BatchNorm degeneracy at batch_size=1 across all models; plus other integration issues as discovered. (Audit scope: D-01)
**Depends on:** Phase 10
**Plans:** 5/5 plans executed

Plans:

- [x] 11-01-PLAN.md — Fix Series2Vec _distance_normalizer gradient break at batch_size=1 (D-03, TDD)
- [x] 11-02-PLAN.md — Add norm parameter to MCL + FCNEncoder (D-02)
- [x] 11-03-PLAN.md — Add norm parameter to TSTCC + TCCEncoder + TemporalContrast (D-02)
- [x] 11-04-PLAN.md — Add norm parameter to Series2Vec + Network + DisjoinEncoder + config (D-02)
- [x] 11-05-PLAN.md — Batch_size=1 gradient flow tests for all 10 models (D-04, D-01)

### Phase 12: Standardize representation_dim Across All Models

**Goal:** Unify dimension naming conventions across all models and add contribution guide for future model additions. Ensure every model exposes a `representation_dim` property that accurately reflects `encode()` output, and eliminate naming inconsistencies.

**Scope** (driven by Opus spec):

1. **Plural → singular, tree-wide.** Rename every `*_dims` identifier to `*_dim` — all are scalar `int` values. Verified: zero `*_dims` occurrences are a list/tuple/Sequence. Ecosystem uses singular (`embedding_dim`, `hidden_size`, `d_model`).
2. **Unify encode-output param name → `representation_dim`.** Currently spelled `output_dims` (5 models) or `representation_dims` (1 model). Unify to `representation_dim` end-to-end (config → model → encoder).
3. **Add `representation_dim` property to all 10 models.** Existing properties fixed (TST returns `hidden_dim * seq_len` → just `hidden_dim`). New properties added (MCL, TimeVAE, TimeNet, RecurrentAE, TS2Vec, AutoTCL, CoST).
4. **Contribution guide.** Document naming conventions for future model additions: param naming rules, property contracts, protocol compliance, test requirements.

**TST Fix:** Property currently returns `hidden_dim × sequence_length` (supervised-head artifact). Fixed to return `hidden_dim` (encode output). Supervised factory computes flatten size inline: `representation_dim * sequence_length`.

**CoST Alignment:** No structural change needed — `output_dims` is the full encoding width (trend + season already split by 2 internally). Rename to `representation_dim`.

**Names that stay:** `latent_dim` (TimeVAE, stochastic bottleneck — distinct concept), `layers` (RecurrentAE, genuine tuple).

**Migration:** Hard rename, no shims. Alpha (a12→a13). Breaking change documented in CHANGELOG.
...
**Spec:** [SPEC.md](SPEC.md) — full rename map, execution steps, verification checklist.

**Contribution Guide:** To be written as part of this phase alongside renames.

### Phase 13: Model Bug Fixes

**Goal:** Fix short-sequence encoder bugs that block UEA/UCR dataset sweep. Scope: Series2Vec DisjoinEncoder crash, TSTCC TemporalContrast crash, TimeVAE stride-too-aggressive on short T, and NaN input guard for all models. Full audit completed — 7 models safe, 3 need fixes.
**Requirements**: [.planning/todos/chronocratic-series2vec-bugs.md](todos/chronocratic-series2vec-bugs.md), TSTCC InsectWingbeat crash, TimeVAE short-sequence validation
**Depends on:** Phase 12
**Plans:** 6/6 plans complete

Plans:

- [x] 13-01-PLAN.md — Shared NaN helpers: zero_fill_padding + masked_reconstruction_loss (Wave 1)
- [x] 13-02-PLAN.md — Series2Vec: kernel config rename + auto-clamp + NaN defense (Wave 2)
- [x] 13-03-PLAN.md — TSTCC: encoder output length + auto-clamp timesteps (Wave 2)
- [x] 13-04-PLAN.md — TimeVAE: encoder output length + auto-clamp conv_stride (Wave 2)
- [x] 13-05-PLAN.md — NaN defense: RecurrentAE, TimeNet, TimeVAE, TST (Wave 3)
- [x] 13-06-PLAN.md — NaN defense: Series2Vec, TSTCC, MCL + dilated verification + integration smoke (Wave 3)
...
*Last updated: 2026-07-09 — scope expanded with Opus spec + contribution guide*
